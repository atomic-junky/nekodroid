...
final favoritesProvider = StateNotifierProvider.autoDispose<
    _FavoritesProviderNotifier, Map<Uri, int>>(
...
class _FavoritesProviderNotifier extends StateNotifier<Map<Uri, int>> {
  ...
  _FavoritesProviderNotifier(this._favsBox)
      : super(
  ...
  Future<void> toggleFavBoxOnly(
      Uri url, DateTime addedAt, NSAnime anime) async {
    // will break on: `2106-02-07 07:28:15.000`
    await _boxUpdate(url.toString(), addedAt.millisecondsSinceEpoch, anime);
    state = state;
  }

  Future<void> _boxUpdate(
      String urlString, int addedAtTimestamp, NSAnime anime) async {
